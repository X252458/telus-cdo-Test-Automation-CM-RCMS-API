Feature: Generation of OAuth Token for RCMS API
  ...
  Scenario: Verify RewardService Credentials(OAuth2 Token) for RCMS
    #Operation 1
    When def accessTokenFeatureCall = call read(PATH_API_OPS+'OAuthAuthentication/getOAuth2TokenRewardService.feature')
    #Response
