Feature: RCMS Activation API - TC01 Call getReturnPenalty operation for Telus Subscriber having ACCESSORYFINANCE
  ...
  Background: Configuration - Set up the authentication, Headers, and params
    ...
    * def payload = read(PATH_API_PAYLOAD +'ResumePenalty/CommonResumePenalty.json')
    * header Content-Type = 'application/json'
    * header Env = karate.properties['karate.apiEnv']
    * header X-System = 'WLS'
 
	
  Scenario: Firing GetResumePenalty API

    Given url ENDPOINT_GET_RESUME_PENALTY_ADJUSTMENTLIST
    And request payload
    When method post
